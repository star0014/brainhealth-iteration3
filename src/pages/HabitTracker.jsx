...
function HabitTracker() {
  ...
  // ── Reminder toast ────────────────────────────────────────────────────────
  // After a successful save, shows a colour-coded contextual toast for 6 seconds
  // based on the user's sleep hours and their saved reminder preferences.
  ...
  return (
    <div className="ht-page">
      ...
      {/* Reminder toast — colour-coded, auto-dismisses after 6 s */}
      {reminderToast && (
        <div
          className="ht-reminder-toast"
          style={{ borderColor: reminderToast.color, background: `${reminderToast.color}12` }}
        >
          <span className="ht-reminder-toast-icon">{reminderToast.icon}</span>
          <span className="ht-reminder-toast-msg">{reminderToast.msg}</span>
          <button className="ht-reminder-toast-close" onClick={() => setReminderToast(null)}>×</button>
        </div>
      )}
      ...
      {view === 'checkin' && (
        <div className="ht-checkin">
          ...
          {/* Smart Reminders guide banner — shown until dismissed */}
          {showReminderGuide && (
            <div className="ht-reminder-guide">
              ...
              <div className="ht-rg-body">
                ...
                <div className="ht-rg-text">
                  <div className="ht-rg-title">Smart Reminders are available!</div>
                  <div className="ht-rg-desc">
                    Get personalised sleep &amp; study reminders after every check-in. You can set a <strong>Study Crunch Mode</strong> before exams to get extra motivational nudges.
                  </div>
                  <div className="ht-rg-features">
                    <span>🌙 Sleep quality tips</span>
                    <span>📚 Study Crunch Mode</span>
                    <span>🎯 Chill / Direct / Hype tone</span>
                  </div>
                  <button className="ht-rg-cta" onClick={() => navigate('/reminders')}>Set up Reminders →</button>
                </div>
              </div>
            </div>
          )}
